...
class Database:
  def __init__(self, PATH_DATABASE, NAME_TABLE):
    self.mysever = pymongo.MongoClient(PATH_DATABASE)
    self.mydatabase = self.mysever["mydatabase"]
    self.mytable = self.mydatabase[NAME_TABLE]
  ...
  def insert_message(self,objection, token, message, time):
    ...
    self.mytable.insert_one(mydict)
  ...
  def delete_one_message_by_token(self, token):
    ...
    self.mytable.delete_one(myquery)
  ### delete
  ...
  def find_last_conservation_chatbot_by_token(self, token):
    ...
    return self.mytable.find_one(mytoken, sort=[( 'time', pymongo.DESCENDING )])
