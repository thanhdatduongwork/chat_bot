...
def read_data(synonyms_dictionary, file_name="Data/Data.xlsx"):
    ...
    for item in set(y):
        ...
        if label.strip() != "None" and label.strip() !="" and label not in list_label \
                and label.strip() != "truy_vấn_ngoài_phạm_vi":
...
def intent_classification(user_query_clean, model, vectorizer, label_list,max_length=25):
    ...
    pred_prob = model.predict(x_feature)
...
if __name__ == '__main__':
    ...
    def processvking(query, synonyms_dictionary):
    # Thực hiện xử lý truy vấn ở đây
        # processed_query = query.upper()  # Ví dụ: chuyển truy vấn thành chữ in hoa
        processed_query = unidecode.unidecode(query)  # Chuyển đổi câu truy vấn thành có dấu
        return processed_query

    # Sử dụng hàm processvking để xử lý truy vấn
    while(True):
        query = input("Nhập truy vấn: ")
        user_query_clean = processvking(query,synonyms_dictionary)
